...
- **제목투(개조식)로 쓰지 마세요.** 명사로 끝나는 조각을 늘어놓으면 대본이 아니라 헤드라인 목록이 됩니다.
  나쁜 예: "증인 44명·참고인 4명 요청" / "민주당 거부로 채택 불발" (제목을 늘어놓은 것)
  좋은 예: "국민의힘은 김승원 후보자 청문회에" → "증인 44명을 요청했지만" → "민주당이 거부해 불발됐습니다"
...
- visual 에는 그 씬 동안 화면에 **그래픽으로** 무엇을 그릴지 적습니다 — 무엇이 크게 보이고, 어느 말에서 무엇이 등장·강조되는지.
  나쁜 예: "국회의사당 전경 스톡", "진행자 클로즈업", "img-1-stat-card.png". 좋은 예: "의석 반원에서 찬성 178석이 채워지고 반대 102석이 뒤따라 채워짐".
  **정치인 얼굴 사진·영상·캐리커처는 지시하지 마세요.** 초상권과 편집 오해 문제가 있어 이름 글자·도형·수치로 갑니다.
  ...
  아래는 지면(에디토리얼) 모양입니다. 값이 다 있을 때만 쓰고, 이름 뒤 * 는 주인공(강조)입니다.
    쟁점: 제목 / 찬성: 주장, 주장 | 반대: 주장, 주장                 (여야·찬반 두 단 — 주장은 자료에 있는 것만)
    오차: 제목 / 이름 42%, 이름 39% / 오차: 3.1 / 출처: 조사기관      (여론조사 — 오차 범위가 자료에 있을 때만)
    누적: 제목 / 찬성, 반대, 모름 / 20대 값 값 값, 60대 값 값 값        (100% 막대 — 몫 합이 100)
    기울기: 제목 / 지난 조사→이번 조사 / 이름 값→값*, 이름 값→값 / 단위: %
    덤벨: 제목 / 2016→2024 / 이름 값→값*, 이름 값→값 / 단위: 석
    전국지도: 제목 / 서울 값, 경기 값, 부산 값 / 빗금: 부산 / 단위: %
    크기: 제목 / 이름 값, 이름 값 / 단위: 조                           (예산처럼 넓이로 크기 비교)
    인물: 이름 / 직함 / 약력, 약력 / “한마디”                           (글자 카드 — 얼굴 없음, 한마디는 실제 발언만)
  ...
  예: "막대: 표결 결과 / 찬성 178, 반대 102 / 출처: 국회 의안정보시스템 + '찬성' 막대가 먼저 자라고 가결선이 그어짐".
- **사진 한 칸 (선택 · 영상 전체에 2곳까지).** 실물이 보이면 이해가 빨라지는 말(특정 단지·건물·장소·제품·문서)이 있는 씬에만,
  ...
  정치 쪽은 **건물·장소·문서·물건만** 됩니다 — 사람이 나오는 사진은 위 규칙대로 지시하지 않습니다.
  예: "막대: 표결 결과 / 찬성 178, 반대 102 / 출처: 국회 의안정보시스템 + 사진(본회의장): 국회 본회의장 전경".
